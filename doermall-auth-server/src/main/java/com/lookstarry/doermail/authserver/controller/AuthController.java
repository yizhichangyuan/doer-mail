package com.lookstarry.doermail.authserver.controller;
...
import com.lookstarry.doermail.authserver.feign.AuthFeignService;
import com.lookstarry.doermail.authserver.feign.MemberFeignService;
...
public class AuthController {
    ...
    public String register(@Valid UserRegisterVo registerVo, BindingResult result,
        ...
        if(StringUtils.isNotEmpty(redisCode) && rightCode.equals(code)){
            ...
            if(r.getCode() == 0){
                // TODO 1默认发的令牌，session=xxx。作用域：当前域（解决子域session的共享问题）
                // TODO 2 redis存储session对应的实体类，应该使用JSON的序列化方式来序列化对象到redis中
                MemberInfoEntity infoEntity = r.getData("data", new TypeReference<MemberInfoEntity>() {
                });
                httpSession.setAttribute("loginUser", infoEntity);
                // 注册成功
    ...
    public String login(UserLoginVo vo, RedirectAttributes redirectAttributes, HttpSession session){
        ...
        if(r.getCode() == 0){
            MemberInfoEntity memberInfo = r.getData("data", new TypeReference<MemberInfoEntity>() {
            });
            session.setAttribute(AuthServerConstant.LOGIN_USER, memberInfo);
